...
export const App: React.FC = () => {
  const getInitialTab = (): NavTab => {
    ...
    if (
      tabParam &&
      ['dashboard', 'accounts', 'wakeup', 'gateway', 'instances', 'inspector', 'settings'].includes(
        tabParam,
      )
    ) {
      return tabParam;
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await Promise.all([
        useGatewayStore.getState().loadGateway(),
        useAccountStore.getState().loadAccounts(),
        useLogStore.getState().loadLogs(),
        useWakeupStore.getState().loadTasks(),
  ...
  return (
    <AppLayout currentTab={currentTab} onSelectTab={setCurrentTab}>
      {!isDesktop() && (
        <div role="status" className="preview-strip">
          <Monitor size={13} />
          <span>Interface preview</span>
          <span className="dim">·</span>
          <span>Open the desktop app for live features.</span>
          <code className="ml-auto">npm run tauri:dev</code>
        </div>
      )}
      {error && (
        <div role="alert" className="notice notice-error error-strip">
          <AlertCircle size={15} />
          <span className="flex-1">{error}</span>
          <button className="icon-button" aria-label="Dismiss error" onClick={clear}>
            <X size={14} />
          </button>
        </div>
      )}
